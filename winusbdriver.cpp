...
#define EP0ADDR         0x01        //0x01     //Write端口0地址，通道0
#define EP1ADDR         0x81        //0x81    //Read 端口1地址，通道1
...
void WinUSBDriver::send(QByteArray byte)
{
    if(is_open)
    {
        ...
        libusb_bulk_transfer(dev_handle,EP0ADDR,(unsigned char *)byte.data(),byte.length(),NULL,0);
    }    
}
